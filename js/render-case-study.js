...
function renderCaseStudy() {
  ...
  headerInner.querySelector(".back-link").textContent = "← Back to work";
  headerInner.querySelector("h1").textContent = project.title;
  headerInner.querySelector(".summary").textContent = project.summary;

  const titleBlock = el("dl", { class: "cs-title-block" }, []);
  [["No.", project.number], ["Year", project.year], ["Role", project.role], ["Material", project.material]]
    .forEach(([label, value]) => {
  ...
  // hero image
  const hero = el("div", { class: "wrap" }, [
    el("div", { class: "cs-hero" }, [mediaEl(project.heroImage, project.title, "16:9")])
  ]);
  ...
  (project.content || []).forEach(section => {
    ...
    const p = el("p", {}, []); p.textContent = section.body;
    const block = el("div", { class: "cs-block" }, [h2, el("div", { class: "body" }, [p])]);
